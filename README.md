...
- **LVN Management**: Fetch LVNs using subaccount's own credentials.
- **LVN Transfer Section**: Transfer LVNs to target account using Vonage Subaccounts Transfer API.
...
- **Outbound Calling**: Make outbound calls using any account owned LVN as the caller ID - no explicit application linking needed.
- **Real-time Call Status**: View real-time call status updates in the UI (via [webhook events](https://developer.vonage.com/en/voice/voice-api/webhook-reference?source=voice#event-webhook)).
- **Response History**: Track and review previous API responses with collapsible history panel.
- **VCR Settings Management**: Advanced settings panel for viewing and managing VCR state and Vonage applications.
...
1. Run the Backend Server, in terminal, `cd backend` and `vcr debug -y -f vcr-backend.yml` to run it in `PORT 3000`.

2. Run the Frontend, in terminal, `cd frontend` and `npm start`

3. In the Backend directory, `/backend/vcr-backend.yml` update the `FRONTEND_URL` value, example `http://localhost:3002/` and rerun the backend server `vcr debug -y -f vcr-backend.yml`
...
2. Deploy the Frontend:
   ...
   2. In the Backend directory, `/backend/vcr-backend.yml` update the `FRONTEND_URL` value, example `https://neru-XXXX.use1.runtime.vonage.cloud` and rerun the backend server `vcr debug -y -f vcr-backend.yml`. You can deploy frontend twice to retrieve it. There's probably a smarter way to do this.
...
1. **Master Account Authentication**
   ...
   - The app validates the master API key against the configured `MASTER_API_KEY` and `MASTER_API_SECRET_KEY` in the backend environment `/backend/vcr-backend.yml`.
...
2. **Subaccount Selection & Automatic Secret Management**
   ...
   - **Automatic Secret Rotation**: The app automatically manages API secrets for the selected subaccount, rotating them as needed while maintaining a maximum of 2 secrets (current API limit).
...
3. **Fetch LVNs (Owned Numbers)**
   ...
   - **LVN Link Status**: LVNs linked to applications display a "Linked" badge.
   - A success message is displayed showing how many LVNs were found and which numbers are available.

4. **Create or Get Subaccount Application**

   - The user clicks **"CREATE OR GET SUBACCOUNT APPLICATION"**.
   - The backend uses the **subaccount's own credentials** to create a new Vonage Application (if one doesn't exist) or retrieve an existing one (Created by this tool only).
...
5. **Make a Call**
   ...
   - The backend uses the subaccount's Application ID and Private Key (which was generated by this tool) to authenticate and send the outbound call via the Vonage Voice API.
   - All errors (authentication, no LVNs, call errors) and success responses are displayed in the UI.

6. **View Call Status (Webhook Events)**
...
When a call is made using the Vonage Voice API, Vonage sends HTTP POST requests to your application's `event_url` webhook with status updates about the call. These events include a variety of statuses and event types that describe the lifecycle and outcome of the call.
...
| `machine`     | The call was answered by a machine (voicemail or IVR). |
| `busy`        | The destination is busy.                               |
| `cancelled`   | The call was cancelled before being answered.          |
| `completed`   | The call has ended (completed normally).               |
| `failed`      | The call could not be completed due to an error.       |
| `rejected`    | The call was rejected by the destination.              |
| `timeout`     | The call timed out before being answered.              |
| `unanswered`  | The call was not answered.                             |
| `transferred` | The call was transferred to another destination.       |
| `dtmf`        | DTMF (touch-tone) input was received during the call.  |
| `voicemail`   | The call was sent to voicemail.                        |
| `error`       | An error occurred during the call.                     |

For a full reference, see the [Vonage Voice API Webhook Reference](https://developer.vonage.com/en/voice/voice-api/webhook-reference?source=voice#event-webhook).
...
### **LVN Transfer Functionality**

- **TRANSFER LVN Button**: Replaces BUY LVN functionality with number transfer capabilities
- **Master-to-Subaccount Transfer**: Transfer existing numbers from selected account to target account
